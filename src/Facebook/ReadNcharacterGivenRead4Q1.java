...
public class ReadNcharacterGivenRead4Q1 {//abcdABCD1234
    ...
    /**
     * buf[] is a user buffer where we strore result to. Basic idea is we create a internal buffer (buff) and every time we read from file,
     * we read from this internal buffer first to avoid expensive system call( here you can imagine sys call is read4()).
     * buffPtr point to next unread char in buff and buffcount indicate length of buff. So when we call read(),
     * we read from buff starting with buffPtr. Once buffPtr == buffCount we finish reading buff and we have to call read4() to refill internal buff and read from buff again.
