...
public class SparseMatrixMultiplication {
     ...
     * for the location of first column and first row in final result matrix. Similarly, the value at [ i ][ j ] of result matrix C,
     * which is C[ i ][ j ] is calculated as:
     * <p>
     ...
     */
    //O(M * N * H) TC H ==> Non zero elements
    public int[][] multiply(int[][] A, int[][] B) {
        ...
        for (int i = 0; i < m; i++) {
            for (int k = 0; k < n; k++) {
                if (A[i][k] != 0) {
                    for (int j = 0; j < nB; j++) {
                        if (B[k][j] != 0)//doesn't affect TC as you still
                            // iterate nb times.
                            C[i][j] += A[i][k] * B[k][j];
                    }
                }
